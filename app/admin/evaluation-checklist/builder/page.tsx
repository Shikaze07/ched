...
function ChecklistBuilderContent() {
  ...
  const [editingSection, setEditingSection] =
    React.useState<Partial<Section> | null>(null);
  const [editingReq, setEditingReq] = React.useState<
    (Partial<Requirement> & { sectionId?: string }) | null
  >(null);
  ...
  const [deletingSectionId, setDeletingSectionId] = React.useState<
    string | null
  >(null);
  ...
  const filteredCmos = Array.isArray(cmos)
    ? cmos.filter((c) => c.programId === selectedProgram)
    : [];
  ...
  const handleDeleteSection = async (id: string) => {
    if (
      !confirm(
        "Are you sure? This will delete all requirements in this section.",
      )
    )
      return;
  ...
  const handleSaveRequirement = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      sortOrder:
        ...
        (sections.find((s) => s.id === editingReq?.sectionId)?.requirements
          .length || 0) + 1,
    };
    ...
    try {
      ...
      if (res.ok) {
        toast.success(
          payload.id ? "Requirement updated" : "Requirement created",
        );
    ...
    <div className="flex flex-1 flex-col p-4 md:p-8 space-y-6">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground">
          Define the evaluation structure for specific programs and CHED
          Memorandum Orders.
        </p>
      </div>
          ...
          <div className="flex-1 space-y-2 w-full">
            ...
            >
              <SelectTrigger className="h-9 shadow-sm">
                <SelectValue
                  placeholder={
                    selectedProgram ? "Select CMO" : "Select a program first"
                  }
                />
              </SelectTrigger>
              <SelectContent>
                {filteredCmos.map((cmo) => (
                  <SelectItem key={cmo.id} value={cmo.id} className="max-w-md">
                ...
                {selectedProgram && filteredCmos.length === 0 && (
                  <div className="p-2 text-sm text-center text-muted-foreground">
                    No CMOs linked to this program.
                  </div>
                )}
              </SelectContent>
            </Select>
                      ...
                      <Button
                        ...
                        onClick={() => {
                          setEditingReq({
                            ...
                            sortOrder: section.requirements.length + 1,
                          });
            ...
            <Card className="border-dashed border-2 py-20 flex flex-col items-center justify-center gap-4">
              ...
              <div className="text-center space-y-1">
                ...
                <p className="text-muted-foreground max-w-xs mx-auto">
                  No sections or requirements have been defined for this CMO
                  yet. Start building it now.
                </p>
              </div>
              <Button
                size="lg"
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-muted/10 border-2 border-dashed rounded-xl gap-6">
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-muted-foreground max-w-sm">
              Select a program and an associated CMO above to start managing the
        ...
        <DialogContent className="sm:max-w-[500px]">
          <form onSubmit={handleSaveSection}>
            <DialogHeader>
              ...
              <DialogDescription>
                Set the section title and numbering (e.g., Section I, Section
                II).
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="grid gap-6 py-2">
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input
                  id="sectionNumber"
                  name="sectionNumber"
                  defaultValue={editingSection?.sectionNumber}
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input
                  ...
                  className="col-span-3 h-10"
                  placeholder="e.g. Legal Basis, Faculty Qualifications"
                  required
                />
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="sortOrder" className="text-right font-medium">
                  Order
                </Label>
                <Input
                  id="sortOrder"
      ...
      <Dialog open={isReqDialogOpen} onOpenChange={setIsReqDialogOpen}>
        <DialogContent className="sm:max-w-[700px]">
          <form onSubmit={handleSaveRequirement}>
            <DialogHeader>
              <DialogTitle className="text-2xl font-bold">
                {editingReq?.id ? "Edit Requirement" : "Add Requirement"}
              </DialogTitle>
              <DialogDescription>
                Define the specific requirement criteria and the evidence that
            ...
            <div className="grid gap-6 py-2">
              <div className="space-y-3">
                <Label
                  htmlFor="description"
                  className="font-bold text-base flex items-center justify-between"
                >
                  Requirement Description
                  <span className="text-xs font-normal text-muted-foreground italic">
                    (HTML/Rich text supported)
                  </span>
                </Label>
                <Textarea
                  id="description"
              ...
              <div className="grid grid-cols-4 items-center gap-10 w-[240px]">
                <Label htmlFor="reqSortOrder" className="font-bold ">
                  Sequence Order
                </Label>
                <Input
                  id="reqSortOrder"
                  name="sortOrder"
                  type="number"
                  defaultValue={editingReq?.sortOrder}
            ...
            <DialogFooter className="mt-6">
              <Button
                ...
                onClick={() => setIsReqDialogOpen(false)}
              >
                Cancel
              </Button>
